...
export class CoordinadoraAdapter {
  ...
  async getLastPedidoNumber(): Promise<number> {
    ...
    try {
      await this.page!.goto(URLS.pedidos, { waitUntil: 'networkidle' });
      await this.page!.waitForTimeout(3000);

      // MUI DataGrid: buscar el primer link en las celdas (es el pedido más reciente)
      const firstLink = await this.page!.$(CoordinadoraSelectors.pedidosList.firstPedidoLink);

      if (firstLink) {
        const numeroText = await firstLink.textContent();
        const numero = parseInt(numeroText?.trim() || '', 10);

        if (!isNaN(numero) && numero > 1000) {
          console.log(`📍 Último pedido: ${numero}`);
          return numero;
        }
      }

      // Alternativa: buscar todos los links y encontrar el número más alto
      const allLinks = await this.page!.$$(CoordinadoraSelectors.pedidosList.firstPedidoLink);
      let maxNumber = 0;
      ...
      if (maxNumber > 0) {
        console.log(`📍 Último pedido (por búsqueda): ${maxNumber}`);
        return maxNumber;
      }

      console.log('⚠️ No se pudo obtener número de pedido, usando fallback');
      return 9597; // Fallback al último conocido

    } catch (error) {
      console.error('❌ Error obteniendo último pedido:', error);
      return 9597; // Fallback
    }
  }
